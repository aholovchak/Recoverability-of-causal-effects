...
setwd("~/Documents/missingness_DAGs/PAPER/aktuell")
...
causal_loop <- foreach(r = 1:runs, .combine='comb', .multicombine=TRUE,
                       .init=list(list(), list()), .packages ="simcausal") %dopar% {
                         setwd("/Users/anastasiiaholovchak/Documents/missingness_DAGs/PAPER/aktuell")
                         source("sim_data_altern_upd.R") # input simulated m-DAG

                         Odat <- simcausal::sim(DAG = Dset_altern, n = 20000, rndseed = r, wide=TRUE)
...
source("sim_data_altern_upd.R") # input simulated TRUE DAG

for(j in 1:num_int){
  compl_Dset_altern <- compl_Dset_altern + action(int_names[j], nodes = get(int_names[j]))
}

# simulate for the intervention
sim_cont <- simcausal::sim(DAG = compl_Dset_altern, actions = int_names, n = 100000, rndseed = 241280)
...
psi_true <- as.data.frame(cbind(matrix(unlist(mget(paste("psi",c(1:num_int),sep=""))),ncol=1),
...
data_final_vl_84 <- data.frame(efv_int = numeric(3*num_int), caus_eff = numeric(3*num_int),
...
################################################################################
setwd("~/Documents/missingness_DAGs/PAPER/aktuell/simulations_results")

#write.csv(data_final_vl_84, "final_vals_vl84_altern_2miss.csv", row.names = FALSE)
#write.csv(data_ci_partial, "ac_alt1_n20000.csv")
...
ggplot(data_final_vl_84, aes(x = efv_int, y = caus_eff)) +
  ...
  scale_color_brewer(palette = "Dark2", labels = c("g-computation, no missing data", "g-computation, available cases, missing data", "true CRC"),
                     name="Estimated on") +
  #scale_color_discrete(labels = c("complete data", "incomplete data", "counterfact. data"),
  #                    name="Estimated on") +
  theme_minimal() +
