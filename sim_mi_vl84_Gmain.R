#' Estimation of \theta_{VL_84} under G_{main} based on multiple imputation
...
Odat_cols <- c("ID", "Sex_0", "Genotype_0", "Age_0", "NRTI_0", "SES_0",
...
Odat2_cols <- c("ID", "Sex", "Genotype", "Age", "NRTI", "Weight",
...
time_function <- function(mat){mat$visit <- seq(1:nrow(mat))-1; return(mat)} # function to add time/visit indicator
...
imputation <- amelia(imp_data,       # data
                     ...
                     bounds = matrix(c(4,0,3,   # matrix with logical bounds for continuous variables: IMPORTANT!(e.g., 4th variable, log(age), has limits of 0 and 3)
                                       6,2,4,
                                       10,0,35),ncol=3,byrow=T)
)
...
for(m in 1:M){
  assign(paste0("idata.",m),
         reshape(imputation$imputations[[m]], v.names=c("Weight","CoMo","Dose","MEMS","C12","VL"),
...
write.csv(results_imp, file = "results_imp_vl84_2miss.csv")

# For Monte Carlo confidence intervals in Appendix D.3
saveRDS(results_list, "mi_Gmain.rds")
