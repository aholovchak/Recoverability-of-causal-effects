#' Simulate a data set corresponding to G_{alt2} (See Appendix A for corresponding SCMs)
...
M <- M +
  ...
  node("MEMS",
       t = 2:t.end,
       distr = "rbern",
       prob = plogis(CoMo[t - 1] * 0.31 + MEMS[t - 1] * 0.31 + 0.71 - 0.5 * SES[0])
  ) +
  node("Weight",
       t = 2:t.end,
       distr = "rnorm_trunc",
...
compl_Dset_altern2 <- simcausal::set.DAG(M)
...
M <- M +
  node("MV",
       t = 0,
       distr = "rbern",
       prob = plogis(-2.95 + 0.1 * SES[0])) +
  node("MV",
       t = 1:t.end,
       distr = "rbern",
       prob = plogis(-2.95 + 0.1 * SES[0] + MV[t - 1])) +
  node("TI",
       t = 0:t.end,
       distr = "rbern",
       prob = 0.05) +
...
Dset_altern2 <- simcausal::set.DAG(M)
