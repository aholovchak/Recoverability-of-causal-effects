# Simulation study for estimation of \theta_{84} under G_{alt2}
...
causal_loop <- foreach(
  ...
  source("sim_data_Galt2.R") # input simulated m-DAG

  Odat <- simcausal::sim(DAG = Dset_altern2, n = 5000, rndseed = r, wide = TRUE)
...
for (j in 1:length(int_names)) { # trick to correctly assign continuous intervention by overwriting default
  for (k in 1:length(1:t_end)) {
    assign(paste("C12.cont.", j, sep = ""), lapply(get(paste("C12.cont.", j, sep = "")), modify_int_const, number = c12_int_values[j]))
  }
}

source("sim_data_altern2_upd.R") # input simulated TRUE DAG

for (j in 1:num_int) {
  compl_Dset_altern2 <- compl_Dset_altern2 + action(int_names[j], nodes = get(int_names[j]))
}

# simulate for the intervention
sim_cont <- simcausal::sim(DAG = compl_Dset_altern2, actions = int_names, n = 100000, rndseed = 241280)
...
################################################################################
write.csv(data_final_vl_84, "final_vals_vl84_altern2_2miss.csv", row.names = FALSE)
